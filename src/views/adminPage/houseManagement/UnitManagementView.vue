...
let getBuildingResponse: any = ref([]);
...
const onClick = async () => {
    ...
    form.buildingName = form.Building.name;
    form.buildingType = form.Building.type;
    console.log(form);
    await addUnitsService(form);
    loading.value = false
    dialog.value = false


    const response: any = await getUnitsService();
...
onMounted(async () => {
    try {
        ...
        // 获取楼栋信息
        getBuildingResponse.value = await getBuildingsService();
        console.log(getBuildingResponse.value);
    } catch (error) {
...
const searchData = async () => {
    ...
    const responseFilter = response.filter((item: any) => {
        ...
        } else if (radio1.value === "buildingName") {
            return item.buildingName.includes(input2.value);
        } else if (radio1.value === "buildingType") {
            return item.buildingType.includes(input2.value);
        }
    });
    ...
    <div class="admin-body-main-table">
        <div class="admin-body-main-table-content">
                ...
                <div>
                    <div class="admin-body-main-table-content-title-search">
                        <el-radio-group v-model="radio1" size="large" fill="#6cf">
                            <el-radio-button label="单元号" value="name" />
                            <el-radio-button label="所属楼栋" value="buildingName" />
                            <el-radio-button label="是否有电梯" value="buildingType" />
                        </el-radio-group>
                        ...
                        <div class="demo-drawer__content">
                            <el-form :model="form">
                                ...
                                <el-form-item label="所属楼栋" :label-width="formLabelWidth">
                                    ...
                                    <el-select v-model="form.Building" value-key="id"
                                        placeholder="Please select activity area">
                                        <!-- 遍历getUserResponse数组 -->
                                        <el-option v-for="item in getBuildingResponse" :key="item.id" :label="item.name"
                                            :value="item" />
            ...
            <el-table :data="tableDataPage" style="width: 100%">
                <el-table-column fixed prop="id" label="#" width="250" />
                <el-table-column prop="name" label="单元号" width="300" />
                <el-table-column prop="buildingName" label="所属楼栋" width="300" />
                <el-table-column prop="buildingType" label="是否有电梯" width="600" />
